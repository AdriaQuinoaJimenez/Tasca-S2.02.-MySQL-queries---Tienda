...
SELECT p.nombre, p.precio, f.nombre AS 'nombre del fabricante' FROM producto p INNER JOIN fabricante f ON f.codigo = p.codigo_fabricante;

-- 22. Llista tots els productes amb nom, preu i nom del fabricant (nombre del fabricante) ordenats alfabèticament.
SELECT p.nombre, p.precio, f.nombre AS 'nombre del fabricante' FROM producto p INNER JOIN fabricante f ON f.codigo = p.codigo_fabricante ORDER BY f.nombre ASC;
...
SELECT p.codigo, p.nombre, f.codigo AS 'codigo fabricante', f.nombre AS 'nombre fabricante' FROM producto p INNER JOIN fabricante f ON f.codigo = p.codigo_fabricante;

-- 24. Retorna el nom, el preu i el nom del fabricant (fabricante), del producte més barat.
SELECT p.nombre, p.precio, f.nombre AS 'fabricant' FROM producto p INNER JOIN fabricante f ON f.codigo = p.codigo_fabricante ORDER BY precio ASC LIMIT 1;

-- 25. Retorna el nom del producte, el preu i el nom del seu fabricant (fabricante), del producte més car.
SELECT p.nombre, p.precio, f.nombre AS 'fabricante' FROM producto p INNER JOIN fabricante f ON f.codigo = p.codigo_fabricante ORDER BY precio DESC LIMIT 1;
...
SELECT p.nombre, p.precio, f.nombre AS 'fabricante' FROM producto p INNER JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE f.nombre = 'Asus' OR f.nombre = 'Hewlett-Packard' OR f.nombre = 'Seagate';
...
SELECT p.nombre, p.precio, f.nombre AS 'fabricante' FROM producto p INNER JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE f.nombre IN('Asus', 'Hewlett-Packard', 'Seagate');
...
SELECT p.nombre, p.precio, f.nombre AS 'fabricante' FROM producto p INNER JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE f.nombre LIKE '%e';
...
SELECT p.nombre, p.precio, f.nombre AS 'fabricante' FROM producto p INNER JOIN fabricante f ON f.codigo = p.codigo_fabricante WHERE f.nombre LIKE '%w%';
...
SELECT f.nombre AS 'fabricante' FROM fabricante f LEFT JOIN producto p ON p.codigo_fabricante = f.codigo WHERE p.codigo_fabricante IS NULL;

-- 36. Retorna tots els productes del fabricant Lenovo. (Sense utilitzar INNER JOIN).
SELECT p.codigo, p.nombre, p.precio, f.codigo AS 'codigo_fabricante' FROM producto p LEFT JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Lenovo';

-- 37. Retorna totes les dades dels productes que tenen el mateix preu que el producte més car del fabricant Lenovo. (Sense usar INNER JOIN).
SELECT p.codigo, p.nombre, p.precio FROM producto p WHERE precio = (SELECT p.precio FROM producto p LEFT JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre = 'Lenovo' ORDER BY p.precio DESC LIMIT 1); 
